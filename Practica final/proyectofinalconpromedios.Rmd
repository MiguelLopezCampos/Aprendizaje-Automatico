...
NUEVO:
Vamos a ver como se distribuyen los missing values en los dataset. Para eso usamos la funciÛn missmap de la librerÌa Amelia
...
##Aplicamos regresiÛn logÌstica
A partir de aquÌ es regresiÛn logÌstica. Uso la funciÛn multinom de la librerÌa nnet.
```{r}
dat <- data.frame(x = data.train.clean[,relevantes.bp], y = label.train$bp)
...
## RegresiÛn logÌstica one vs all
Como sabemos, regresiÛn logÌstica es un modelo de clasificaciÛn binomial, es decir, solo funciona con 2 distintas clases. Por otro lado, existe una manera de adaptar regresiÛn logÌstica a problemas multiclase o multinomial. Este mÈtodo (entre otros) se llama 'One vs All'. Su funcionamiento se basa en lo siguiente:

Para cada clase calculamos un modelo de regresiÛn logÌstica, en el que esa clase representar· una clase y el resto de clases una clase com˙n, es decir, adaptamos el problema a un problema binomial. Una vez calculados los modelos para cada clase de esta manera, a la hora de predecir, predecimos cada dato con todos los modelos. øDe quÈ clase ser· entonces el dato? Ser· de la clase cuyo modelo nos devuelva una mayor probabilidad.

Procedamos a la implementaciÛn de este modelo.
```{r}
...
Nuestra funciÛn de predicciÛn directamente nos dar· la clase a la que pertenece cada dato.
```{r}
